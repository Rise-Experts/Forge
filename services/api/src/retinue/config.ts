 ...
 * is two sets of defaults and two spellings, and the platform's already handles the `RETINUE_*` → `RETINUE_*` / `AGENTKIT_*`
...
export const loadServiceConfig = (env: Readonly<Record<string, string | undefined>> = process.env): ServiceConfig => {
  ...
  const schema = (env["RETINUE_SCHEMA"] ?? env["RETINUE_SCHEMA"])?.trim();
