...
test("clean tree yields no violations", () => {
  const dir = fixture({
    ...
    "frontend/src/x.ts": `import type { RunEvent } from "@forge/agentkit";\n`,
...
test("planted violations are all caught", () => {
  const dir = fixture({
    ...
    "frontend/src/bad.ts": `import { runtime } from "@forge/agentkit";\n`,
    ...
    "backend/src/persistence/bad.ts": `import { PostgresStore } from "@forge/postgres";\n`,
...
test("R8: a generic package must not import an integration package", () => {
  const dir = fixture({
    "backend/src/tools/bad.ts": `import { createShareFlowToolProvider } from "@forge/shareflow";\n`,
    "frontend/src/bad.ts": `import type { PostDraft } from "@forge/shareflow";\n`,
    "server/src/bad.ts": `import { x } from "@forge/shareflow/dist/index.js";\n`,
  });
  try {
...
test("R8 survives renaming the package, where R5 would not", () => {
  ...
  // `@forge/shareflow` only because of how it is spelled. This asserts R8 is the rule doing the
  // work, so a future rename cannot silently drop the guarantee.
  const dir = fixture({ "backend/src/bad.ts": `import { x } from "@forge/shareflow";\n` });
  try {
...
test("R10: an import must be declared in the package's own manifest", () => {
  const dir = fixture({
    "shareflow/package.json": JSON.stringify({ dependencies: { "@forge/agentkit": "*" } }),
    ...
    "shareflow/src/tools/ok.ts": `import { defineTool } from "@forge/agentkit";\nimport { join } from "node:path";\n`,
  });
  try {
...
test("real imports are still found after sanitizing", () => {
  ...
  const dir = fixture({
    "frontend/src/bad.ts": [
      ...
      `const note = "we import from @forge/agentkit";`,
      `import { runtime } from "@forge/agentkit";`,
      ``,
    ].join("\n"),
  });
  try {
