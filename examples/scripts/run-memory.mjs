 ...
 *
 *   FORGE_MODEL_API_KEY=sk-… node scripts/run-memory.mjs
 *
...
if (!process.env.FORGE_MODEL_API_KEY) {
  console.error("✗ FORGE_MODEL_API_KEY is required — the model is the one thing this mode cannot fake.");
  process.exit(2);
}
/**
...
console.log(`
  forge example — single process, in memory

    page      http://localhost:${port}/
    graphql   http://localhost:${port}/graphql
    model     ${process.env.FORGE_MODEL_ID ?? "gpt-4o"}
