...
const tenantId = process.env.RETINUE_MCP_TENANT ?? process.env.RETINUE_MCP_TENANT ?? "mcp-demo";
const principalId = process.env.RETINUE_MCP_PRINCIPAL ?? process.env.RETINUE_MCP_PRINCIPAL ?? "mcp-user";
...
const context = {
  ...
  roleIds: (process.env.RETINUE_MCP_ROLES ?? process.env.RETINUE_MCP_ROLES ?? "editor").split(",").map((r) => asId(r.trim())),
  locale: "en",
  timezone: "UTC",
