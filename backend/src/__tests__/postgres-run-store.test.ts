 ...
 * real against `FORGE_TEST_PG_URL` in CI and reports why it was skipped otherwise.
 */
...
describe("concurrent claim across two connections", () => {
  ...
  if (!PG_URL) {
    it("[skipped: FORGE_TEST_PG_URL unset — PGlite is one embedded instance, so a two-connection test here would be meaningless]", () => {
