...
const validEnv = {
  FORGE_DATABASE_URL: "postgres://user:pw@localhost:5432/agentkit",
...
describe("configuration", () => {
  ...
  it("rejects a malformed URL by naming the variable and what was wrong", () => {
    for (const [variable, value] of [
      ["FORGE_DATABASE_URL", "mysql://nope"],
      ["FORGE_REDIS_URL", "http://nope"],
    ] as const) {
      const error = (() => {
        try {
  ...
  it("rejects a non-positive concurrency rather than booting a worker that consumes nothing", () => {
    for (const value of ["0", "-1", "1.5", "many"]) {
      expect(() => loadConfig({ ...validEnv, FORGE_WORKER_CONCURRENCY: value })).toThrow(
        ConfigurationError,
      );
    }
    ...
    expect(() => loadConfig({ ...validEnv, FORGE_WORKER_CONCURRENCY: "8" })).not.toThrow();
  });

  it("validates the schema mode against the library's union", () => {
    for (const mode of ["auto", "plan", "off"]) {
      expect(loadConfig({ ...validEnv, FORGE_SCHEMA_MODE: mode }).schemaMode).toBe(mode);
    }
    expect(() => loadConfig({ ...validEnv, FORGE_SCHEMA_MODE: "migrate" })).toThrow(/must be one of/);
  });
  ...
  it("takes the environment as an argument, so nothing global is read", () => {
    ...
    expect(process.env["FORGE_DATABASE_URL"]).toBeUndefined();
  });
});
...
describe("startup provisioning", () => {
  const bootAgainst = async (sql: SqlExecutor, mode: string) => {
    ...
    const result = await boot({
      env: { ...validEnv, FORGE_SCHEMA_MODE: mode },
      connect: async () => ({ sql }),
...
describe("runnable commands", () => {
  it("reports configuration problems before looking for the app module", async () => {
    ...
    // Ordering matters: a deployment that removed FORGE_DATABASE_URL must be told about *that*, not
    // about FORGE_APP_MODULE. An earlier version loaded the app module first and reported the wrong
    // variable.
    expect(error).toBeInstanceOf(ConfigurationError);
