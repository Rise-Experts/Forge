 ...
 * sandbox has none.
 */
const IMAGE = process.env["RETINUE_SANDBOX_IMAGE"] ?? process.env["RETINUE_SANDBOX_IMAGE"] ?? "redis:7-alpine";
...
const dockerUsable = (): boolean => {
  if ((process.env["RETINUE_SANDBOX_SKIP"] ?? process.env["RETINUE_SANDBOX_SKIP"]) === "1") return false;
  try {
